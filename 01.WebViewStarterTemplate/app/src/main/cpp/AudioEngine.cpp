...
bool AudioEngine::openStreams() {
    ...
    // Start both streams and fail the native start transaction if either route
    // cannot actually enter the running state.
    auto inputStart = inputStream_->requestStart();
    auto outputStart = outputStream_->requestStart();
    if (inputStart != oboe::Result::OK || outputStart != oboe::Result::OK) {
        LOGE("Failed to start streams: input=%s output=%s",
             oboe::convertToText(inputStart), oboe::convertToText(outputStart));
        closeStreams();
        return false;
    }
...
oboe::DataCallbackResult AudioEngine::onAudioReady(
    ...
    // In the required native tier the Oboe output is the ALSA loopback source.
    // Never silently switch to a software monitor when this route is unavailable.
...
Java_com_webviewtemplate_webviewtemplate_audio_OboeEngine_nativeCreate(JNIEnv*, jobject) {
...
JNIEXPORT void JNICALL
Java_com_webviewtemplate_webviewtemplate_audio_OboeEngine_nativeSetParam(
    ...
    switch (effectId) {
        ...
        case 4: // Pitch
            ...
            break;
        case 5: // Gain
            if (paramId==0) p.gain.store(value, std::memory_order_relaxed);
            else if (paramId==1) p.gainEnabled.store(value > 0.5f, std::memory_order_relaxed);
            break;
        case 6: // Echo
            if (paramId==0) p.echo.store(value, std::memory_order_relaxed);
            else if (paramId==1) p.echoEnabled.store(value > 0.5f, std::memory_order_relaxed);
            break;
...
JNIEXPORT jboolean JNICALL
Java_com_webviewtemplate_webviewtemplate_audio_OboeEngine_nativeIsRunning(
...
JNIEXPORT void JNICALL
Java_com_webviewtemplate_webviewtemplate_audio_OboeEngine_nativeSetInjectionMode(
...
JNIEXPORT void JNICALL
Java_com_webviewtemplate_webviewtemplate_audio_OboeEngine_nativeSetInputDevice(
...
JNIEXPORT void JNICALL
Java_com_webviewtemplate_webviewtemplate_audio_OboeEngine_nativeSetMonitorCallback(
...
JNIEXPORT jint JNICALL
Java_com_webviewtemplate_webviewtemplate_audio_OboeEngine_nativeGetInputSessionId(
    JNIEnv*, jobject, jlong handle) {
    auto* engine = reinterpret_cast<micplugin::AudioEngine*>(handle);
